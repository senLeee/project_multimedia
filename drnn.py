...
def dRNN(cell, inputs, rate, scope='default'):
    ...
    dilated_inputs = [tf.concat(0,inputs[i * rate:(i + 1) * rate]) for i in range(dialated_n_steps)]
    # building a dialated RNN with reformated (dilated) inputs
    dilated_outputs, _ = rnn.rnn(
        cell, dilated_inputs,
    ...
    splitted_outputs = [tf.split(split_dim=0, num_split = rate, value=output)
...
def _contruct_cells(hidden_structs, cell_type):
    ...
    for hidden_dims in hidden_structs:
        if cell_type == "RNN":
            cell = tf.nn.rnn_cell.BasicRNNCell(hidden_dims)
        elif cell_type == "LSTM":
            cell = tf.nn.rnn_cell.BasicLSTMCell(hidden_dims)
        elif cell_type == "GRU":
            cell = tf.nn.rnn_cell.GRUCell(hidden_dims)
        cells.append(cell)
...
def _rnn_reformat(x, input_dims, n_steps):
    ...
    x_reformat = tf.split(split_dim=0, num_split = n_steps, value=x)
